...
public class Turtle implements Runnable, ActionListener, MouseListener, MouseMotionListener, KeyListener, ComponentListener, MouseWheelListener
{
    ...
    public static void main(String[] a) {
        TurtleThread_1 turtle1 = new TurtleThread_1("red", 0, -40);
        TurtleThread_2 turtle2 = new TurtleThread_2("orange",0 , -40);
        TurtleThread_3 turtle3 = new TurtleThread_3("brown", 0, -40);
    ...
    static  class TurtleThread_1 extends Thread {
        private
            Turtle turtle;
            String color;
            double x, y;
        public
            ...
            int thickness = 4;
        public TurtleThread_1(String color, double x, double y) {
            this.color = color;
            this.x = x;
            this.y = y;
        }
        public void run() {
            ...
            while (round_S < 2) {//draw candle flame from right to left
                if (round_S == 0) {
                    //middle flame
                    turtle.width(thickness + 6);
                    turtle.dot();
                }
                turtle.up();
                for (int j = 0; j < angle_max; j++) { //round_S = 0 == right
                    turtle.forward(1);            //round_S = 1 == left
                    turtle.left(1);
                    if (j == 89) turtle.left(j++);
                }
                round_S++;
            }
            turtle.right(35);
    ...
    static  class TurtleThread_2 extends Thread {
        private
            Turtle turtle;
            String color;
            double x, y;
        public
            int angle = 30, angle_max = angle * 3;
            int thickness = 4;
        public TurtleThread_2(String color, double x, double y) {
            this.color = color;
            this.x = x;
            this.y = y;
        }
        public void run() {
            ...
            turtle.right(angle*2); // adjust angle, position to draw frame
            turtle.up();
            turtle.right(angle_max*2);
            turtle.forward(70);
            turtle.right(angle_max*1.15);
            turtle.forward(40);
            try {
                Thread.sleep(100);
            } catch (InterruptedException e) {
    ...
    static  class TurtleThread_3 extends Thread {
        private
        Turtle turtle;
        String color;
        double x, y;
        ...
        int angle = 30, angle_max = angle * 3;
        int thickness = 4;
        public TurtleThread_3(String color, double x, double y) {
            this.color = color;
            this.x = x;
            this.y = y;
        }
        public void run() {
            ...
            turtle.right(angle_max*2);
            turtle.forward(70);
            turtle.right(angle_max*2.5);
            turtle.down();
            int round_B = 0; // B = Big
            for(int i =0; i < 2; i++) {
                while (round_B < 2) {//draw frame circle from right to left
                    if (round_B == 1 && i == 0) turtle.width(thickness-1);
                    else turtle.width(thickness + 2);
                ...
                if (i == 0) { // move to the upper after finish lower circle
                    turtle.width(thickness + 2); //i = 0 == lower circle
                    turtle.left(angle_max + 60);   // i = 1 == upper circle
                    turtle.forward(175);
                    round_B = 0;
                    turtle.right(angle_max + 60);
                }
            }
            turtle.left(angle+15);
            turtle.up();
            turtle.forward(165);
            turtle.right(angle+45);
            turtle.width(thickness + 2);
            turtle.down();
            turtle.forward(175);
            turtle.up();
            turtle.left(90);
